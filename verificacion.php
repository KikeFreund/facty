...
function verificarSesion() {
    ...
    if ($resultado->num_rows === 1) {
        ...
        $nueva_expiracion = date('Y-m-d H:i:s', time() + (60 * 60 * 24 * 7)); // 7 días
        $stmt = $conn->prepare("UPDATE sesiones SET expiracion = ? WHERE token = ?");
        $stmt->bind_param("ss", $nueva_expiracion, $token);
        $stmt->execute();
